...
Hint: just ignore that Intellij can not resolve `System.out.println("..")`. Its a bug, and will disapear after a while.
...
- exception for more then 2 decimal places by adding "expected" to the `@Test` annotation (junit4) or `assertThrows()` in junit5
