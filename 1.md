...
### Exercise 1
- exception for more then 2 decimal places by ~~adding "expected" to the `@Test` annotation (junit4) or~~ using `assertThrows()` from junit5
### Exercise 2
...
- the less experience developer takes the keyboard (the "driver"), the other  one ("navigator") checks if the driver works according to the TDD-process and the requirements.
